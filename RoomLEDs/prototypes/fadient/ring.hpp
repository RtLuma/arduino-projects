...
struct node {
    ...
    node *next;
    node() { lum=pos=37; }
    node(int8_t l, uint16_t p) { lum=l; pos=p; }
};

struct Ring {
        node* *ring; uint8_t nodes;
        
        Ring() { nodes = 0; }
        Ring(uint8_t P) { nodes = P; }
            
        void populate (uint8_t desiredNodes) { 
            if (desiredNodes > nodes) {
                for (uint8_t i=nodes; i < desiredNodes; i++) ring[i] = new node();
                nodes = desiredNodes;
            }
            else {
                for (uint8_t i=nodes; i > desiredNodes; i--) delete ring[i];
                nodes = desiredNodes;
            }
        }
