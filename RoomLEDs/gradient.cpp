...
void gradient(uint8_t* pixelA, uint8_t* pixelB) {
  ...
  printColor(pixelA[0],pixelA[1],pixelA[2]); printf(" -> "); printColor(pixelB[0],pixelB[1],pixelB[2]);  printf(" over %d steps: ",SECTION);
  r = pixelA[0] << 8; g = pixelA[1] << 8; b = pixelA[2] << 8;
  r_step = (((uint16_t)pixelB[0] << 8) - r)/SECTION;
  g_step = (((uint16_t)pixelB[1] << 8) - g)/SECTION;
  b_step = (((uint16_t)pixelB[2] << 8) - b)/SECTION;
