...
void loop() {
  ...
  if (!digitalRead(BUTTON_1)) {
    delay(300);
    for (int j = 0; digitalRead(BUTTON_1); j += 2) {
      if (j > kolor(mode, -1)) j = 0;
      for (int i = 1; i < strip.numPixels(); i++) {
        float val = double(kolor(mode, -1)) * (float(i) / float(strip.numPixels() - 1));
        strip.setPixelColor(i, kolor(mode, val + j));
      }
      strip.show();
      if (!digitalRead(BUTTON_2)) {
        delay(300);
        mode++;
        if (mode == kolor(-1, 0)) mode = 0;
      }
    }
    delay(300);
  }
  ...
  if (count > kolor(mode,-1))  count = 0;

  //delay(20);
}

uint32_t kolor(int8_t mo, int i) {
  if (mo == 2) return Sea(i);
  if (mo == 1) return Sunset(i);
  if (mo == 0) return Rain(i);
  if (mo < 0) return 3;
  else return 0;
}
...
void showSpectrum() {
  ...
  for (int i = 0; i < SPEC; i++) {
    ...
    float val = double(kolor(mode,-1)) * (float(i) / float(SPEC));
    ...
    if (Lspectrum[i] > 90) {
      ...
      uint32_t Lcol = kolor(mode,val + count), Lcol2 = strip.getPixelColor(Lpos);
    ...
    if (Rspectrum[i] > 90) {
      ...
      uint32_t Rcol = kolor(mode,val + count), Rcol2 = strip.getPixelColor(Rpos);
...
uint32_t Sea(int i) {
  ...
  if (i < 0) return LOOP;
  if (i > LOOP - 1) return Sea(i % LOOP);
  if (i > 509) return strip.Color(i % 255, 255 - (i % 255) / 2, 255 - (i % 255));            //cyan -> orange
  if (i > 255) return strip.Color(255 - (i % 255), i % 255, 255);                //purple -> cyan
  return strip.Color(255, 128 - (i / 2), i);    //orange -> purple
}
...
uint32_t Sunset(int i) {
  ...
  if (i > 509) return strip.Color(255 - (i % 255), 128 - (i % 255) / 2, (i % 255));   //orange -> blue
  if (i > 255) return strip.Color(255, (i % 255) / 2, 0);      //red -> orange
  return strip.Color(i, 0, 255 - i);                            //blue -> red
}
