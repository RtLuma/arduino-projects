	...
	void main(void)
		...
		char read_buffer[21];   /* Buffer to store the data received              */
		const char read_buffer2[21] = "EEPROM WRITE SUCCESS";
		int  bytes_read = 0;    /* Number of bytes read by the read() system call */
 		int i = 0;
		int ret=1;
		 
		while (ret) {
			bytes_read = read(fd,&read_buffer,21); /* Read the data                   */
			
			for(i=0;i<bytes_read;i++) printf("%c",read_buffer[i]);

			ret = strcmp(read_buffer, read_buffer2);
