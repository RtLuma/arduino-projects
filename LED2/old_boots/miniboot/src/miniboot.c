...
static inline bool isCrcOk(const uint8_t i2c_address) {
  ...
  for (uint16_t pos = 0; pos < length + 1; pos += 2) {  // length +1 in case of odd number lengths

    if (pos >= length) break;

    uint16_t data = getWordFromSource(i2c_address, pos + start_address);
    if (pos == length - 1) data &= 0xFF00;

    uint8_t to_little_endian[2];
    to_little_endian[0] = static_cast<uint8_t>(data >> 8);
    to_little_endian[1] = static_cast<uint8_t>(data);

    crc32(reinterpret_cast<const void*>(&to_little_endian[0]), 2, &table[0], &crc);

    // uint8_t toggle_led_every_x_bytes = 128;
    // if (0 == pos % toggle_led_every_x_bytes) LED_TOGGLE();
  }

  uint32_t expected_crc = static_cast<uint32_t>(getWordFromSource(i2c_address, application_crc_expected_index)) << 16;
  expected_crc |= static_cast<uint32_t>(getWordFromSource(i2c_address, application_crc_expected_index + 2));

  if (crc == expected_crc) status = true;
  return status;
}
...
static inline void writeFlashFromI2C(const uint8_t i2c_address, uint16_t& application_start) {
  ...
  for (
    uint16_t pos = SPM_PAGESIZE - (static_cast<uint16_t>(writes + 1) * static_cast<uint16_t>(SPM_PAGESIZE)) % length;
    pos < SPM_PAGESIZE;
    ++pos
    ) {
...
[[noreturn]] static inline void leaveBootloader(uint16_t& application_start) {
...
static inline bool isReflashNecessary(uint32_t& i2c_application_timestamp) {
  ...
  i2c_application_timestamp = static_cast<uint32_t>(getWordFromSource(source_i2c_address_for_program, timestamp_application_byte_offset)) << 16;
  i2c_application_timestamp |= static_cast<uint32_t>(getWordFromSource(source_i2c_address_for_program, timestamp_application_byte_offset + 2));
...
int main() {
  ...
  // && isCrcOk(source_i2c_address_for_program)
    ...
    uint16_t address_in_external_eeprom = getWordFromSource(source_i2c_address_for_program, application_start_address_byte_offset);
