...
//  pinMode(A0, INPUT);
  ...
  //  v = random(255);
  //  hue = random(255);
  //  for (int i = 0; i < NUMPIXELS; i++) ring.setPixelColor(i, ring.Color(0, 0, 0));
  //  ring.show();
  ring.clear();
  for (int i = 0; i < NUMPIXELS; i++) ring.setPixelColor(i, ring.Color(255, 255, 255));
  ring.show();
}
...
void loop() {
  ...
  for (int i = 0; i < NUMPIXELS; i++) {
    auto t = T+(i<<4);
    ring.setPixelColor(i, ring.Color(rainbow(t), rainbow(t - 85), rainbow(t - 127)));
  }
  ring.show();
  ...
  //  p = (p + v);
  //  uint8_t pos = map(p, 0, 0xFFFF, 0, NUMPIXELS);
